namespace DhrMaes.Storage.Core.Plugins
{
	...
	internal class PluginLoader
	{
		...
		public IStorageProvider LoadProvider(string configPath)
		{
			...
			var loadedConfig = JsonSerializer.Deserialize(File.ReadAllText(configPath), providerType) as IStorageProviderConfig;
			if (loadedConfig is null)
			{
				throw new InvalidOperationException($"Failed to load provider config '{providerTypeName}'.");
			}

			var providerInstance = loadedConfig.CreateProvider();
